...
//namespace BytexDigital.RGSM.Node.Application.Services
//{
...
//        public IQueryable<Node> GetNode(string id)
//        {
//            return _applicationDbContext.Nodes.Where(x => x.Id == id);
//        }

//        public async Task<IQueryable<RGSM.Domain.Entities.Node>> GetLocalNodeAsync() =>
//            GetNode(await _nodeSettingsService.GetValueAsync<string>(NodeSettingsService.KEY_SETTING_NODEID));

//        public async Task<IQueryable<RGSM.Domain.Entities.Node>> CreateFromLocalNodeAsync()
//        {
//            string nodeId = await GetLocalNodeIdAsync();

//            if (await GetNode(nodeId).AnyAsync()) throw new InvalidOperationException("The local node already exists in the shared database.");
